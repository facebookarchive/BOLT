# The test performs a BB reordering with unsupported 
# instruction jrcxz. Reordering works correctly with the
# following options: None, Normal or Reverse. Other strategies 
# are completed with Assertion `isIntN(Size * 8 + 1, Value).
# The root cause is the distance between BB where one contains 
# jrcxz instruction.
# Example: OpenSSL
...
# RUN:    -debug -print-disasm -print-all -v=2 -lite=false \
# RUN:    -reorder-blocks=cache+ -reorder-functions=hfsort \
# RUN:    -split-functions=2 -split-all-cold -split-eh -dyno-stats \
# RUN:    -print-finalized -max-funcs=200 | FileCheck %s

# CHECK: const: Assertion `isIntN(Size * 8 + 1, Value) && \
...
# FDATA: 1 bn_from_mont8x/1 160 1 bn_sqrx8x_internal 0 0 56
