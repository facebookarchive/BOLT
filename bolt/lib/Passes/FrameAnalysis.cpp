//===--- Passes/bolt/Passes/FrameAnalysis.h -------------------------------------------===//
//
...
#include "bolt/Core/Parallelism.h"
...
void FrameAnalysis::cleanAnnotations() {
  ...
  Parallelism::WorkFuncTy CleanFunction = [&](BinaryFunction &BF) {
    for (BinaryBasicBlock &BB : BF) {
      for (MCInst &Inst : BB) {
  ...
  Parallelism::runOnEachFunction(
      BC, Parallelism::SchedulingPolicy::SP_INST_LINEAR, CleanFunction,
      Parallelism::PredicateTy(nullptr), "cleanAnnotations");
}
...
void FrameAnalysis::clearSPTMap() {
  ...
  Parallelism::WorkFuncTy ClearFunctionSPT = [&](BinaryFunction &BF) {
    std::unique_ptr<StackPointerTracking> &SPTPtr = SPTMap.find(&BF)->second;
    SPTPtr.reset();
  };

  Parallelism::PredicateTy SkipFunc = [&](const BinaryFunction &BF) {
    return !BF.isSimple() || !BF.hasCFG();
  };

  Parallelism::runOnEachFunction(
      BC, Parallelism::SchedulingPolicy::SP_INST_LINEAR, ClearFunctionSPT,
...
void FrameAnalysis::preComputeSPT() {
  ...
  // Run SPT in parallel
  Parallelism::WorkFuncWithAllocTy ProcessFunction =
  ...
  Parallelism::PredicateTy SkipPredicate = [&](const BinaryFunction &BF) {
    return !BF.isSimple() || !BF.hasCFG();
  };

  Parallelism::runOnEachFunctionWithUniqueAllocId(
      BC, Parallelism::SchedulingPolicy::SP_BB_QUADRATIC, ProcessFunction,
