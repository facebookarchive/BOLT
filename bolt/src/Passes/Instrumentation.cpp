...
void Instrumentation::createAuxiliaryFunctions(BinaryContext &BC) {
  ...
  // indirect call. We pass it to createInstrumentedIndCallHandlerEntryBB(), which
  // will check if a pointer to runtime library traffic accounting function was
  // initialized (it is done during initialization of runtime library).
  // If it is so - calls it. Then this routine returns to normal execution by
  // jumping to exit BB.
  ...
  IndCallHandlerExitBBFunction = createSimpleFunction(
      "__bolt_instr_ind_call_handler_func",
      BC.MIB->createInstrumentedIndCallHandlerEntryBB(
          Summary->IndCallCounterFuncPtr, IndCallHandlerExitBB->getSymbol(), &*BC.Ctx));
